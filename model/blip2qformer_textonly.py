...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        self.Qformer_netlist, self.query_tokens_netlist = self.init_Qformer(bert_name, num_query_token, self.graph_encoder.num_features, cross_attention_freq)
        self.Qformer.resize_token_embeddings(len(self.tokenizer))
        self.Qformer_netlist.resize_token_embeddings(len(self.tokenizer))
    ...
    def contrast_global(self, features_graph, features_text, features_graph_all, features_text_all, return_sim=False):
        ...
        # print(logits_per_graph)
        # print(logits_per_graph.dtype)
        loss_graph = F.cross_entropy(logits_per_graph, labels)
        loss_text = F.cross_entropy(logits_per_text, labels)
        loss = (loss_graph + loss_text) / 2
    ...
    def forward(self, batch):
        ...
        graph, text, mask, netlist_text, netlist_mask, rtl_text, rtl_mask = batch
        netlist_output = self.Qformer_netlist.bert(netlist_text, attention_mask=netlist_mask, return_dict=True)
        netlist_feats = self.text_proj(netlist_output.last_hidden_state[:, 0:2, :])

        batch_node = netlist_feats # shape = [B, 2, D], use netlist as graph_feats
        graph_feats = batch_node
        batch_size = batch_node.shape[0]
        batch_mask = torch.ones((batch_size, 2), dtype=torch.long, device=batch_node.device)
