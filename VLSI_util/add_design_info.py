"""
The script adds design information to each netlist graph. e.g. area, and power.
"""
...
parser.add_argument("--graph_path", type=str, default="/home/weili3/VLSI-LLM-Graph/VLSI_util/RTLCoder26532.pt", help="graph path")
...
for graph in graphs['graphs']:
    ...
    syntheisis_effort = graph.synthesis_efforts
    # get the design info
    if rtl_index in design_info:
        if design_info[rtl_index][syntheisis_effort][0] is not None:
            if float(design_info[rtl_index][syntheisis_effort][0]) > 0:
                graph.area = float(design_info[rtl_index][syntheisis_effort][0])
                graph.power = float(design_info[rtl_index][syntheisis_effort][1])
...
torch.save({"graphs": new_graphs, "rtl_id_list": graphs['rtl_id_list']}, new_path)
