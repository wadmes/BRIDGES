...
system_setting = """You are a hardware expert. Provide the power approximation of a Verilog module.
...
def main(args):
    ...
    with torch.no_grad():
        for graph in tqdm.tqdm(graph_list):
            ...
            {"role": "user", "content": f"""
                 Provide a power approximation of the following Verilog module. 
    **{args.use_data} code**
    {data}
    **synthesis effort:**
    {graph.synthesis_efforts}.
      Please reply with only the power number. The power is:
             """}
            ]
            ...
            print(f"Predicted: {reply}, True: {graph.power}")
            results["netlist_id"].append(graph.netlist_id)
            results["label"].append(graph.power)
            ...
            if re.search(p, reply) is not None:
                ...
                print(reply_num)
                mape = abs((float(reply_num) - float(graph.power)) / float(graph.power))
    ...
    # save as csv
    with open(f"{dataset_name}_{args.use_data}_power.csv", "w") as f:
