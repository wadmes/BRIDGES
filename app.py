...
def netlist_caption(netlist_path, prompt, temperature):
    ...
    molca.llm_tokenizer.padding_side = 'right'
    prompt_batch = molca.llm_tokenizer([prompt,],
                                       truncation=False,
                                       padding='longest',
                                       add_special_tokens=True,
                                       return_tensors='pt',
                                       return_attention_mask=True).to(args.devices)
    is_graph_token = prompt_batch.input_ids == molca.llm_tokenizer.additional_special_tokens[2]
    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16):
        text = molca.generate(samples, temperature=temperature, max_length=256, num_beams=5, do_sample=True)[0]
    return text
